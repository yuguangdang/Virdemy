...
class Course extends CI_Controller { 
	...
	// 	if (!$this->upload->do_upload('userfile')) {
    //         $data = array('course_id' => $course_id, 'page'=>'');
	// 		$this->load->view('template/header', $data);
	// 		$this->session->set_flashdata('vid-error', $this->upload->display_errors());
	// 		$this->load->view('course/course_edit/'.$course_id, $data);
	// 		$this->load->view('template/footer');
	// 	} else {
	// 		$file_id = $this->file_model->upload_video($this->upload->data('file_name'), $this->upload->data('full_path'), $course_id);
	// 		$this->session->set_flashdata('vid-message', 'Course video successfully uploaded!');
	// 		$data = array(
	// 			'error' => "", 
	// 			'course_id' => $course_id,
	// 			'page' => 'create_course'
